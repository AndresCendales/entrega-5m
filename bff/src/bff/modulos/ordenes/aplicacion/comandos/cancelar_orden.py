from bff.src.bff.seedwork.aplicacion.comandos import Comando
from bff.src.bff.modulos.ordenes.aplicacion.dto import ProductoDTO, OrdenDTO, UbicacionDTO
from .base import CrearOrdenBaseHandler
from dataclasses import dataclass
from bff.src.bff.seedwork.aplicacion.comandos import ejecutar_commando as comando

from bff.src.bff.modulos.ordenes.dominio.entidades import Orden
from bff.src.bff.seedwork.infraestructura.uow import UnidadTrabajoPuerto
from bff.src.bff.modulos.ordenes.aplicacion.mapeadores import MapeadorOrden
from bff.src.bff.modulos.ordenes.infraestructura.repositorios import RepositorioOrdenes, RepositorioEventosOrdenes

@dataclass
class CancelarOrden(Comando):
    fecha_creacion: str
    fecha_actualizacion: str
    id: str
    client_id: str
    origen: UbicacionDTO 
    destino: UbicacionDTO 
    tipo: str 
    productos: list[ProductoDTO]

class CancelarOrdenHandler(CancelarOrden):
    
    def handle(self, comando: CancelarOrden):
        orden_dto = OrdenDTO(
                fecha_actualizacion=comando.fecha_actualizacion
            ,   fecha_creacion=comando.fecha_creacion
            ,   id=comando.id
            ,   client_id=comando.client_id
            ,   origen=comando.origen
            ,   destino=comando.destino
            ,   tipo=comando.tipo
            ,   productos=comando.productos)

        orden: Orden = self.fabrica_ordenes.crear_objeto(orden_dto, MapeadorOrden())
        orden.crear_orden(orden)

        repositorio = self.fabrica_repositorio.crear_objeto(RepositorioOrdenes)
        repositorio_eventos = self.fabrica_repositorio.crear_objeto(RepositorioEventosOrdenes)

        UnidadTrabajoPuerto.registrar_batch(repositorio.agregar, orden, repositorio_eventos_func=repositorio_eventos.agregar)
        UnidadTrabajoPuerto.commit()


@comando.register(CancelarOrden)
def ejecutar_comando_crear_orden(comando: CancelarOrden):
    handler = CancelarOrdenHandler()
    handler.handle(comando)
    